...
class ClinicalVisitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ClinicalVisit
        fields = [
            'id', 'student', 'student_university_id', 'student_name',
            'clinician', 'clinician_username', 'symptoms', 'diagnosis',
            'advice', 'treatment', 'note', 'lab_used', 'lab_note',
            'pharmacy_used', 'medication_given', 'visit_date', 'is_locked',
        ]
        read_only_fields = ['clinician', 'visit_date', 'is_locked']

    def get_student_name(self, obj):
        return f"{obj.student.first_name} {obj.student.last_name}"

    def validate(self, attrs):
        # Use getattr to handle partial updates (PATCH requests)
        ...
        if lab_used and not lab_note:
            raise serializers.ValidationError({"lab_note": "Add a lab note if lab was used."})

        if pharmacy_used and not medication_given:
            raise serializers.ValidationError({"medication_given": "Add medication details if pharmacy was used."})
